...
export const AnimatedTrustBadge = () => {
  ...
  return <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full 
      ...
      {/* Sparkle decorations */}
      <Sparkles className="absolute -left-1 top-1/2 -translate-y-1/2 w-3 h-3 text-asper-gold/50 animate-pulse" style={{
      animationDelay: '0.2s'
    }} />
      <Sparkles className="absolute -right-1 top-1/2 -translate-y-1/2 w-3 h-3 text-asper-gold/50 animate-pulse" style={{
      animationDelay: '0.7s'
    }} />
      ...
      <div className="relative flex-shrink-0">
        ...
        <div className={`
          ...
          `}>
          <IconComponent className="w-5 h-5 text-asper-merlot drop-shadow-sm" strokeWidth={2} />
        </div>
      ...
      <div className="flex items-center gap-1.5 ml-1">
        {trustBadges.map((_, idx) => <div key={idx} className={`
              ...
              ${idx === currentIndex ? 'bg-asper-gold shadow-[0_0_6px_rgba(212,175,55,0.8)] scale-125' : 'bg-asper-ivory/30 hover:bg-asper-ivory/50'}
            `} />)}
      </div>
